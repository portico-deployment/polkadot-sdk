...
impl Discovery {
	...
	pub async fn add_known_peer(&mut self, peer: PeerId, addresses: Vec<Multiaddr>) {
		tokio::time::timeout(
			...
			self.kademlia_handle.add_known_peer(peer, addresses),
		)
		.await
		.expect("`add_known_peer()` took more than 10 seconds");
	}
	...
	pub async fn get_value(&mut self, key: KademliaKey) -> QueryId {
		tokio::time::timeout(
			std::time::Duration::from_secs(10),
			self.kademlia_handle.get_record(RecordKey::new(&key.to_vec()), Quorum::One),
		)
		.await
		.expect("`get_value()` took more than 10 seconds")
	}
