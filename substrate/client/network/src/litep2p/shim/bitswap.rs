...
//! Shim for litep2p's Bitswap implementation to make it work with `sc_network`.

use crate::service::traits;

use futures::StreamExt;
use litep2p::protocol::libp2p::bitswap::{
	BitswapEvent, BitswapHandle, BlockPresenceType, Cid, Config, ResponseType, WantType,
};
...
impl<Block: BlockT> BitswapServer<Block> {
	...
	pub fn new(
		...
		let mut bitswap = Self { client, handle };

		(Box::pin(async move { bitswap.run().await }), config)
	}

	async fn run(mut self) {
		log::error!(target: LOG_TARGET, "starting bitswap server");
