...
impl<'a> sp_wasm_interface::Riscv for HostContext<'a> {
	fn execute(
		...
		let mut execute_config = polkavm::ExecutionConfig::default();
		execute_config.set_gas(polkavm::Gas::MAX);

		let engine = polkavm::Engine::new(&config).unwrap();
		let module = polkavm::Module::from_blob(&engine, &module_config, &program).unwrap();
		...
		let outcome = match func.call_ex(&mut context, (), execute_config) {
			Ok(_) => RiscvExecOutcome::Ok,
			Err(polkavm::ExecutionError::Trap(_)) => RiscvExecOutcome::Trap,
			Err(polkavm::ExecutionError::OutOfGas) => RiscvExecOutcome::OutOfGas,
			Err(err) => {
